...
/// query for biggest and smallest msgid
...
try
{
	...
	for (int i=0; i<hits.length;i++)
	{
	    ...
	    //out.println(explanation.getDetails()[0]+"<br>");
		TokenStream tokenStream = TokenSources.getAnyTokenStream(searcher.getIndexReader(), hits[i].doc, "content", analyzer);
	    TextFragment[] frag = highlighter.getBestTextFragments(tokenStream, doc.get("content"), false, 10);
	    String data = "";
	    for (int j = 0; j < frag.length; j++) {
	        if ((frag[j] != null) && (frag[j].getScore() > 0)) {
	      	  data=data+" "+frag[j].toString();
	        }
	      }
	    ...
	    //dataMap.put("content", data.replaceAll("(\r\n)\\1+", "\n").replace("<B>", "\"").replace("</B>", "\"").replaceAll("<[^>]*>", ""));
	    //dataMap.put("content", data.replaceAll("(\r\n)\\1+", "\n").replace("<B>", "").replace("</B>", "").replaceAll("<[^>]*>", "")+
	    //		Jsoup.parse(doc.get("content")).text().replaceAll("\r", "").replaceAll("&nbsp;"," ").replaceAll(" +", " ").replaceAll("(\\s*\n)\\1+", "\n").replaceAll("(\\s*\n)\\1+", "\n").replaceAll(">", ""));
	    dataMap.put("content",
	    		Jsoup.parse(doc.get("content")).text().replaceAll("\r", "").replaceAll("&nbsp;"," ").replaceAll(" +", " ").replaceAll("(\\s*\n)\\1+", "\n").replaceAll("(\\s*\n)\\1+", "\n").replaceAll(">", ""));
